        ...
        <div class="joe_archive">
          <div class="joe_archive__title">
            ...
            <div class="joe_archive__title-title">
              ...
              <span>篇与</span>
              <span class="muted ellipsis"><?php echo $this->archiveTitle(); ?></span>
              <span>的结果</span>
            </div>
          </div>
          ...
          <?php if ($this->have()) : ?>
            <ul class="joe_archive__list joe_list" data-wow="<?php $this->options->JList_Animate() ?>">
              <?php while ($this->next()) : ?>
                <?php 
                // 获取文章字段，兼容 PHP 7.4-8.5
                $postMode = isset($this->fields->mode) ? $this->fields->mode : '';
                
                if ($postMode === "default" || $postMode === "") : 
                ?>
                  <li class="joe_list__item wow default">
                    <div class="line"></div>
                    <a href="<?php $this->permalink() ?>" class="thumbnail" title="<?php $this->title() ?>" target="_self" rel="noopener noreferrer">
                      <?php 
                      $thumbnails = _getThumbnails($this);
                      $thumbnailUrl = is_array($thumbnails) && isset($thumbnails[0]) ? $thumbnails[0] : '';
                      ?>
                      <img width="100%" height="100%" class="lazyload" src="<?php _getLazyload() ?>" data-src="<?php echo $thumbnailUrl; ?>" alt="<?php $this->title() ?>" />
                    ...
                    <div class="information">
                      <a href="<?php $this->permalink() ?>" class="title" title="<?php $this->title() ?>" target="_self" rel="noopener noreferrer">
                        <?php $this->title() ?>
                      </a>
                      <a class="abstract" href="<?php $this->permalink() ?>" title="文章摘要" target="_self" rel="noopener noreferrer"><?php _getAbstract($this) ?></a>
                      <div class="meta">
                        <ul class="items">
                ...
                <?php elseif ($postMode === "multiple") : ?>
                  <li class="joe_list__item wow multiple">
                    ...
                    <a href="<?php $this->permalink() ?>" class="thumbnail" title="<?php $this->title() ?>" target="_self" rel="noopener noreferrer">
                      <?php 
                      $images = _getThumbnails($this);
                      // 确保有足够的图片，最多显示3张
                      for ($x = 0; $x < 3; $x++) : 
                        $imageUrl = is_array($images) && isset($images[$x]) ? $images[$x] : '';
                        if (empty($imageUrl)) continue;
                      ?>
                        <img width="100%" height="100%" class="lazyload" src="<?php _getLazyload() ?>" data-src="<?php echo $imageUrl; ?>" alt="<?php $this->title() ?>" />
                      <?php endfor; ?>
                    </a>
                    <div class="meta">
                      <ul class="items">
