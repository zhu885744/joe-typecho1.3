document.addEventListener('DOMContentLoaded', () => {
    ...
    /* 初始化首页列表功能 */
    {
            ...
            if (_.mode === 'default') {
                return `
                    <li class="joe_list__item wow default">
                        <div class="line"></div>
                        <a href="${_.permalink}" class="thumbnail" title="${_.title}" target="_self" rel="noopener noreferrer">
                        ...
                        <div class="information">
                            <a href="${_.permalink}" class="title" title="${_.title}" target="_self" rel="noopener noreferrer">
                                <span class="badge" style="display: ${_.type === 'sticky' ? 'inline-block' : 'none'}">置顶</span>${_.title}
                            </a>
                            <a class="abstract" href="${_.permalink}" title="文章摘要" target="_self" rel="noopener noreferrer">${_.abstract}</a>
                            <div class="meta">
                                <ul class="items">
                                    <li>${_.created}</li>
                                    <li>${_.views} 阅读</li>
                                ...
                                <div class="last" style="display: ${_.category.length ? 'flex' : 'none'}">
                                    ...
                                    <a class="link" target="_self" rel="noopener noreferrer" href="${_.category.length && _.category[0].permalink}">${_.category.length && _.category[0].name}</a>
                                </div>
                            </div>
                        </div>
                    </li>
        ...
        let queryData = {
            page: parseInt(window.Joe.CURRENT_PAGE) || 1,
        ...
        const syncTabState = () => {
            ...
            if (activeItem.length && activeLine.length) {
                activeItem.addClass('active').siblings().removeClass('active');
                activeLine.css({ left: activeItem.position().left, width: activeItem.width() });
            }
        };

        /* 首次加载只同步Tab下划线（PHP已输出首屏文章，不需要AJAX拉取） */
        syncTabState();

        /* 通用 AJAX 加载（mode: 'append'=追加 或 'replace'=替换列表） */
        const pushDom = (mode = 'append') => {
            return new Promise((reslove) => {
                ...
                $.ajax({
                    ...
                    success(res) {
                        ...
                        $('.joe_load').removeAttr('loading');
                        $('.joe_load').html('查看更多');
                        // 如果接口返回 totalPages 且已到最后一页，隐藏查看更多
                        if (res.totalPages && res.currentPage >= res.totalPages) {
                            $('.joe_load').hide();
                        } else {
                            $('.joe_load').show();
                        }
                        ...
                        if (wow !== 'off' && wow && typeof WOW !== 'undefined') {
                            new WOW({ boxClass: 'wow', animateClass: `animated ${wow}`, offset: 0, mobile: true, live: true, scrollContainer: null }).init();
                        }
                        // 重初始化懒加载
                        if (typeof lazyload === 'function' || (window.lazyload && typeof window.lazyload === 'function')) {
                            try { window.lazyload(); } catch(e) {}
                        }
        ...
        $('.joe_index__title-title .item').on('click', async function () {
            ...
            syncTabState();
            // 切换排序后，隐藏 PHP 渲染的 pageNav（因为 pageNav 是基于默认排序生成的）
            $('.joe_pagination').hide();
            // 确保查看更多按钮可用
            $('.joe_load').show();
            pushDom('replace').then(() => {
                const listTop = $('.joe_index__list').offset().top - $('.joe_header').height() - 20;
                window.scrollTo({ top: listTop, behavior: 'smooth' });
            });
        });

        /* 查看更多按钮（追加模式） */
        $(document).on('click', '.joe_load', async function () {
            if ($(this).attr('loading')) return;
            queryData.page++;
            let length = await pushDom('append');
            if (length > 0) {
