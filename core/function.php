...
{
  $db = Typecho_Db::get();
  $result = $db->fetchRow($db->select('views')->from('table.contents')->where('cid = ?', $item->cid));
  $views = isset($result['views']) ? $result['views'] : 0;
  if ($type) echo number_format($views);
  else return number_format($views);
}
...
{
  $db = Typecho_Db::get();
  $result = $db->fetchRow($db->select('agree')->from('table.contents')->where('cid = ?', $item->cid));
  $agree = isset($result['agree']) ? $result['agree'] : 0;
  if ($type) echo number_format($agree);
...
function _getAsideAuthorMotto()
{
  ...
  // 确保数组不为空
  if (empty($JMottoRandom) || !is_array($JMottoRandom)) {
    echo '';
    return;
  }
...
function _getAbstract($item, $type = true)
{
  ...
  if ($item->password) {
    ...
    if (isset($item->fields) && $item->fields->abstract) {
...
function _getThumbnails($item)
{
  ...
  if (isset($item->fields) && $item->fields->thumb) {
  ...
  if (count($result) < 3) {
    ...
    if ($custom_thumbnail) {
      ...
      // 确保数组不为空
      if (!empty($custom_thumbnail_arr)) {
        for ($i = 0; $i < 3; $i++) {
          $random_key = array_rand($custom_thumbnail_arr, 1);
          $result[] = ($custom_thumbnail_arr[$random_key] ?? '') . "?key=" . mt_rand(0, 1000000);
        }
      } else {
        for ($i = 0; $i < 3; $i++) {
...
{
    // 检查配置是否开启
    $config = Helper::options()->JAside_Author_Nav ?? 'off';
    if ($config && $config !== "off") {
        $limit = intval($config);
        $limit = $limit > 0 ? $limit : 5; // 兜底默认显示5篇
        
        // 全局上下文获取当前文章作者ID（需在文章页/列表页生效）
        $archive = Typecho_Widget::widget('Widget_Archive');
        if (!$archive->is('single') && !$archive->is('index')) {
            return; // 非文章页/列表页不显示
        }
        $authorId = $archive->authorId ?? 0;
        if (empty($authorId)) {
            return; // 无作者ID时退出
        }
        ...
        try {
            ...
            if (!empty($result)) {
                ...
                foreach ($result as $item) {
                    // 通过组件解析文章数据，生成完整链接
                    $contentWidget->push($item);
                    $title = htmlspecialchars($contentWidget->title ?? '');
                    $permalink = $contentWidget->permalink ?? '#'; // 组件生成的完整链接
