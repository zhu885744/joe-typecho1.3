<aside class="joe_aside">
  ...
  <?php if ($this->options->JAside_History_Today === 'on') : ?>
    ...
    // 使用Typecho标准查询方式，兼容1.3.0
    $query = Typecho_Widget::widget('Widget_Contents_Post_Recent', 'pageSize=20');
    
    // 收集符合条件的文章
    while ($query->next()) {
        $postDate = date('m-d', $query->created);
        
        // 判断是否为往年今日（排除今年发表的文章）
        if ($postDate === $today && $query->created < strtotime(date('Y-01-01 00:00:00'))) {
            $historyTodaylist[] = array(
                "title" => htmlspecialchars($query->title, ENT_QUOTES, 'UTF-8'),
                "permalink" => $query->permalink,
                "date" => date('Y', $query->created) . ' ' . $postDate
            );
            
            if (count($historyTodaylist) >= 10) {
                break;
            }
        }
    }
    ...
    <?php if (count($historyTodaylist) > 0) : ?>
      <section class="joe_aside__item today">
        ...
        <ul class="joe_aside__item-contain">
          <?php foreach ($historyTodaylist as $historyItem) : ?>
            <li class="item">
              <div class="tail"></div>
              <div class="head"></div>
              <div class="desc">
                <time datetime="<?php echo str_replace(' ', '-', $historyItem['date']) ?>"><?php echo $historyItem['date'] ?></time>
                <a href="<?php echo $historyItem['permalink'] ?>" title="<?php echo $historyItem['title'] ?>">
                  <?php echo $historyItem['title'] ?>
                </a>
              </div>
            </li>
          <?php endforeach; ?>
  ...
  <?php if ($this->options->JAside_Hot_Num && $this->options->JAside_Hot_Num !== 'off') : ?>
    <section class="joe_aside__item hot">
      ...
      <?php $this->widget('Widget_Contents_Hot@Aside', 'pageSize=' . $this->options->JAside_Hot_Num)->to($hotItem); ?>
      <ol class="joe_aside__item-contain">
        <?php if ($hotItem->have()) : ?>
          <?php $index = 1; ?>
          <?php while ($hotItem->next()) : ?>
            <li class="item">
              <a class="link" href="<?php $hotItem->permalink(); ?>" title="<?php $hotItem->title(); ?>">
                <i class="sort"><?php echo $index; ?></i>
                <img width="100%" height="130" class="image lazyload" src="<?php _getLazyload(); ?>" data-src="<?php echo _getThumbnails($hotItem)[0]; ?>" alt="<?php $hotItem->title() ?>" />
                <div class="describe">
                  <h6><?php $hotItem->title(); ?></h6>
                  <span><?php $hotItem->views(); ?> 阅读 - <?php $hotItem->date('m/d'); ?></span>
                </div>
              </a>
            </li>
            <?php $index++; ?>
  ...
  <?php if ($this->options->JAside_Newreply_Status === 'on' && $this->options->JCommentStatus !== 'off') : ?>
    <section class="joe_aside__item newreply">
      ...
      <?php $this->widget('Widget_Comments_Recent', 'ignoreAuthor=true&pageSize=5')->to($commentItem); ?>
      <ul class="joe_aside__item-contain">
        <?php if ($commentItem->have()) : ?>
          <?php while ($commentItem->next()) : ?>
            <li class="item">
              <div class="user">
                <img width="40" height="40" class="avatar lazyload" src="<?php _getAvatarLazyload(); ?>" data-src="<?php _getAvatarByMail($commentItem->mail) ?>" alt="<?php $commentItem->author(false) ?>" />
                <div class="info">
                  <div class="author"><?php $commentItem->author(false) ?></div>
                  <span class="date"><?php $commentItem->date('Y-m-d'); ?></span>
                </div>
              </div>
              <div class="reply">
                <a class="link" href="<?php _parseAsideLink($commentItem->permalink); ?>">
                  <?php _parseAsideReply($commentItem->content); ?>
                </a>
              </div>
            </li>
          <?php endwhile; ?>
  ...
  <?php if ($this->options->JADContent) : ?>
    <?php
    $adContent = $this->options->JADContent;
    $adParts = explode("||", $adContent);
    if (count($adParts) >= 2 && !empty($adParts[0]) && !empty($adParts[1])) {
        $adImage = trim($adParts[0]);
        $adLink = trim($adParts[1]);
    ?>
    <a class="joe_aside__item advert" target="_blank" rel="noopener noreferrer nofollow" href="<?php echo $adLink; ?>" title="广告">
      <img class="lazyload" width="100%" src="<?php _getLazyload() ?>" data-src="<?php echo $adImage; ?>" alt="广告" />
