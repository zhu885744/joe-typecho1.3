  ...
  <div class="joe_header__above">
    <div class="joe_container">
      ...
      <nav class="joe_header__above-nav">
        ...
        <?php 
        // 使用标准 Typecho 循环方式获取页面数据
        $pages = $this->widget('Widget_Contents_Page_List');
        
        // 创建页面数据数组
        $pageItems = array();
        
        // 使用标准的 Typecho 循环方式遍历页面
        if ($pages->have()) {
            while ($pages->next()) {
                $pageItems[] = array(
                    'slug' => $pages->slug,
        ...
        // 获取导航最大数量设置
        $navMaxNum = $this->options->JNavMaxNum ?: 5;
        ?>
        
        <?php if (count($pageItems) <= $navMaxNum) : ?>
          ...
          <?php endforeach; ?>
        <?php else : ?>
          <?php foreach (array_slice($pageItems, 0, $navMaxNum) as $item) : ?>
            <a class="item <?php echo $this->is('page', $item['slug']) ? 'active' : '' ?>" href="<?php echo $item['permalink'] ?>" title="<?php echo $item['title'] ?>"><?php echo $item['title'] ?></a>
          <?php endforeach; ?>
          <div class="joe_dropdown" trigger="hover" placement="60px" style="margin-right: 15px;">
            ...
            <nav class="joe_dropdown__menu">
              <?php foreach (array_slice($pageItems, $navMaxNum) as $item) : ?>
      ...
      <!-- 搜索表单 -->
      <form class="joe_header__above-search" method="post" action="<?php $this->options->siteUrl(); ?>" onsubmit="return validateSearch(this)">
        <input id="search-input-1" maxlength="16" autocomplete="off" placeholder="请输入关键字..." name="s" value="<?php echo $this->is('search') ? $this->archiveTitle(' &raquo; ', '', '') : '' ?>" class="input" type="text" required/>
  ...
  <div class="joe_header__below">
    <div class="joe_container">
      ...
      <nav class="joe_header__below-class">
        ...
        // 构建树结构
        foreach ($parentMap as $mid => $cat) {
            if ($cat['parent'] == 0) {
                $categoryTree[$mid] = &$parentMap[$mid];
            } else {
                if (isset($parentMap[$cat['parent']])) {
                    $parentMap[$cat['parent']]['children'][$mid] = &$parentMap[$mid];
                }
            }
        }
        ...
        <?php foreach ($categoryTree as $cat) : ?>
          <?php if (empty($cat['children'])) : ?>
            <a class="item <?php echo $this->is('category', $cat['slug']) ? 'active' : '' ?>" href="<?php echo $cat['permalink']; ?>" title="<?php echo $cat['name']; ?>"><?php echo $cat['name']; ?></a>
          <?php else : ?>
          ...
          <div class="joe_dropdown" trigger="click">
            <div class="joe_dropdown__link">
              <svg class="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="15" height="15">
                ...
                <path d="M512 255.843s21.49-5.723 21.49-31.306S512 191.882 512 191.882c-70.65 0-127.921 57.273-127.921 127.922 0 3.322.126 6.615.375 9.875.264 3.454 14.94 18.116 37.044 14.425 22.025-3.679 26.6-21.93 26.6-21.93-.028-.788-.06-1.575-.60-2.37.001-35.325 28.637-63.961 63.962-63.961z" />
              </svg>
  ...
  <div class="joe_header__searchout">
    <div class="joe_container">
      <div class="joe_header__searchout-inner">
        <form class="search" method="post" action="<?php $this->options->siteUrl(); ?>" onsubmit="return validateSearch(this)">
          <input id="search-input-2" maxlength="16" autocomplete="off" placeholder="请输入关键字..." name="s" value="<?php echo $this->is('search') ? $this->archiveTitle(' &raquo; ', '', '') : '' ?>" class="input" type="text" required/>
          <button type="submit" class="submit">搜索</button>
        </form>
        ...
        <?php if ($tags->have()) : ?>
          <div class="title">
            <svg class="icon" viewBox="0 0 1445 1024" xmlns="http://www.w3.org/2000/svg" width="22" height="22">
              <path d="M1055.021 277.865a348.762 348.762 0 0 1 348.401 348.341c0 178.96-136.132 327.68-311.778 346.172l-758.603 2.35A291.238 291.238 0 0 1 42.165 683.79a291.238 291.238 0 0 1 273.227-290.334 369.242 369.242 0 0 1 368.4-351.292 370.568 370.568 0 0 1 344.184 236.905c9.336-.783 18.19-1.205 27.045-1.205zM683.791 156.25a255.036 255.036 0 0 0-254.735 254.796V507h-95.955a177.032 177.032 0 0 0-176.79 176.791 177.032 177.032 0 0 0 176.79 176.85h721.98a234.677 234.677 0 0 0 234.316-234.435 234.616 234.616 0 0 0-234.316-234.255a234.616 234.616 0 0 0-234.315 234.315v18.07H706.56v-18.07A348.4 348.4 0 0 1 915.817 307.2a255.578 255.578 0 0 0-232.026-151.01z" />
            </svg>标签搜索
          </div>
          <ul class="cloud">
            <?php $colors  = array(
    ...
    <ul class="joe_header__slideout-menu panel-box">
      ...
      <li>
        ...
        <ul class="slides panel-body panel-box">
          <?php 
          // 重新构建分类树用于侧边栏
          $sidebarCategories = $this->widget('Widget_Metas_Category_List');
          $sidebarCategories->to($sidebarCategory);
          
          $sidebarTree = array();
          $sidebarMap = array();
          
          while ($sidebarCategory->next()) {
              $sidebarMap[$sidebarCategory->mid] = array(
                  'mid' => $sidebarCategory->mid,
                  'name' => $sidebarCategory->name,
                  'slug' => $sidebarCategory->slug,
                  'permalink' => $sidebarCategory->permalink,
                  'parent' => $sidebarCategory->parent,
                  'levels' => $sidebarCategory->levels,
                  'children' => array()
              );
          }
          
          foreach ($sidebarMap as $mid => $cat) {
              if ($cat['parent'] == 0) {
                  $sidebarTree[$mid] = &$sidebarMap[$mid];
              } else {
                  if (isset($sidebarMap[$cat['parent']])) {
                      $sidebarMap[$cat['parent']]['children'][$mid] = &$sidebarMap[$mid];
                  }
              }
          }
          ?>
          
          <?php foreach ($sidebarTree as $cat) : ?>
            <?php if (empty($cat['children'])) : ?>
              <li>
                <a class="link <?php echo $this->is('category', $cat['slug']) ? 'current' : '' ?>" href="<?php echo $cat['permalink']; ?>" title="<?php echo $cat['name']; ?>"><?php echo $cat['name']; ?></a>
              </li>
            <?php else : ?>
              <li>
                <div class="link panel <?php echo $this->is('category', $cat['slug']) ? 'current' : '' ?>">
                  <a href="<?php echo $cat['permalink']; ?>" title="<?php echo $cat['name']; ?>"><?php echo $cat['name']; ?></a>
                  <svg class="icon" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg" width="13" height="13">
                    <path d="M624.865 512.247L332.71 220.088c-12.28-12.27-12.28-32.186 0-44.457 12.27-12.28 32.186-12.28 44.457 0l314.388 314.388c12.28 12.27 12.28 32.186 0 44.457L377.167 848.863c-6.136 6.14-14.183 9.211-22.228 9.211s-16.092-3.071-22.228-9.211c-12.28-12.27-12.28-32.186 0-44.457l292.155-292.16z" />
                  </svg>
                </div>
      ...
      <li>
        ...
        <ul class="slides panel-body">
          <?php foreach ($pageItems as $item) : ?>
            <li>
              <a class="link <?php echo $this->is('page', $item['slug']) ? 'current' : '' ?>" 
